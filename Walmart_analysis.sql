...
'/Users/mrbis/OneDrive/Desktop/intern/learning/SQL/walmart_sales/walmart_sales_data.csv'
INTO TABLE sales
...
-- Update time_of_day column based on time range
UPDATE sales
SET time_of_day = (
    CASE 
...
-- Update day_name column based on the day of the week
UPDATE sales
...
-- Update month_name column based on the month of the date
UPDATE sales
SET month_name = MONTHNAME(date);

---------------- Exploratory Data Analysis (EDA) ----------------------

-- 1. How many distinct cities are present in the dataset? ANS :- 3
...
FROM sales GROUP BY month_name ORDER BY total_cogs DESC LIMIT 1;
...
FROM sales GROUP BY branch HAVING SUM(quantity) > (SELECT AVG(quantity) FROM sales) ORDER BY quantity DESC LIMIT 1;
